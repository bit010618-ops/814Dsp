...
- 按 `full/source/exam_training_manifest.json` 补齐尚缺的 20 道真题训练与详解，继续优先第 3 章余下 5 道；所有题干按原卷保留，跨章节题按依赖规则放置，最终统一回填“详解见 P.××”。
...
- 尚缺 20 道训练/详解、真题图重绘、全书分页、答案索引回填和附录尚未完成；当前机器的 Edge/Chromium headless GPU 缓存故障暂时阻断新的 PDF 视觉验收。
