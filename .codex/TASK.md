...
- 第一章的 41 个清单题和第六章的 13 个清单题已完成题号级覆盖核对。下一步补齐第四章经审计确认缺失的 9 题训练与详解；所有题干按原卷保留，跨章节题按依赖规则放置，最终统一回填“详解见 P.××”。
...
- 旧的“尚缺 15 道”计数来自训练页头数量，不能再作为题目数量使用。第一章和第六章已经题号级证明完整；第四章仍缺 9 题。真题图重绘、全书分页、答案索引回填和附录尚未完成；当前机器的 Edge/Chromium headless 截图故障暂时阻断新的网页截图验收。
