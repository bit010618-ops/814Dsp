...
- 第一章的 41 个清单题已完成题号级覆盖核对。接着对第 4、6 章执行同样的题号级审计，再补齐经审计确认缺失的训练与详解；所有题干按原卷保留，跨章节题按依赖规则放置，最终统一回填“详解见 P.××”。
...
- 旧的“尚缺 15 道”计数来自训练页头数量，不能再作为题目数量使用。第一章已经题号级证明完整；第 4、6 章仍待同一审计。真题图重绘、全书分页、答案索引回填和附录尚未完成；当前机器的 Edge/Chromium headless 截图故障暂时阻断新的网页截图验收。
