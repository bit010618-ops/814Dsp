...
> 2026-08-30 第二章源图补回：源第 248 页的非因果三抽头平均器与延时一拍后的因果稳定实现，原正文仅保留了公式和结论、缺少结构对照。现以两张可编辑 SVG 补回，所有信号标记使用 MathJax，图元采用真实信号流拓扑。首次 A4 静态渲染发现外部 SVG CSS 未被 WeasyPrint 应用而造成黑块；已从根因修正为各图元自带明确填充和描边属性，并以唯一箭头标记 ID 消除冲突。重新静态渲染并目视检查两页，图形不黑、无裁切、箭头及延时/求和/增益块均完整；第二章与全书相关回归 `60 passed`。最终全书候选 PDF 未替换，仍待继续覆盖审计后一次性导出验收。

> 2026-08-30 源页覆盖核对继续：补回第六章巴特沃斯低通原型的频率归一化定义 \(p=s/\Omega_c\)、归一化极点和 \(H(p)\)，公式前明确说明其用于把不同截止频率统一到单位截止频率下设计；补回第八章 CD 音频兼容工程案例，明确 44.1 kHz／16 bit、制作链路中的 48/96/192 kHz 与 24 bit，以及非整数倍重采样和位深降低的职责区别。两处均先以定向回归失败锁定，再修正后通过；相关完整装配回归为 `58 passed`。当前仅更新中间 HTML，候选 PDF 未替换，仍待继续逐页图形和源内容核对后统一导出、回填与桌面/移动端视觉验收。

> 2026-08-29 公式用途说明审计与候选稿更新：逐条审计发现“标题含线性”会使无关公式被误标为“系统线性条件”。已用失败回归覆盖 z 变换时移、DFT 循环时移、线性相位 FIR 对称条件、因果余弦 z 变换、理想延时关系，以及书末二阶 FIR 的零点因式分解和差分方程；随后以公式结构优先于标题语义的规则修复。全套回归 `30 passed`，新 PDF 为 465 页 A4，回填后的答案页码无占位符；实际渲染检查第 21、460 页无源码、黑块或裁切，且公式前均有准确中文名称及用途说明。已更新 `full/outputs/数字信号处理讲义_正文图形修复候选.pdf`；该候选稿仍不是全项目最终验收。

> 2026-08-29 GitHub 推送确认：已将本轮第三章关系图、第五至第八章正文补全、真题自然续排、公式用途说明修正及 463 页候选 PDF 推送至 `origin/main`。远端前进范围为 `67c6b64..8f5ec57`；推送命令实际返回成功。下一步继续按源页逐章核查其余尚未完成的正文和图形，不把当前候选稿误报为整项目最终完成。

> 2026-08-29 任务日志提交待推送：本地提交 `97198d6` 仅记录上述已验证的推送结果；再次推送时 Git 凭据失效并返回 `SEC_E_NO_CREDENTIALS`，因此该日志提交尚未同步至远端。不得误报该提交已上传；凭据恢复后仅重试 `git push origin main`。

> 2026-08-29 全书最终候选 PDF 重新导出并验收：额度恢复后，已以最新首轮 PDF 的实际答案锚点重新回填训练题和附录 E 的“详解见 P.××”，再经本地 MathJax 静态渲染导出。`full/outputs/数字信号处理讲义_正文图形修复候选.pdf` 已更新为 463 页 A4；文本／链接核验确认八章训练区、附录 F、150 个答案锚点齐全，旧分区标题与两类页码占位符为零。实际渲染复核第六章公式总表：双线性变换的三条公式均以“频率映射关系（用于在模拟和数字频率之间建立非线性对应）”说明，未再误称系统线性条件；同时抽查训练页、附录 F 与页眉页脚，无公式源码、黑块或裁切。

> 2026-08-29 全书真题自然续排与公式说明修正：移除了真题区的强制新页、题块不可拆分和 230 mm 最小高度，使短题按自然版面续排，候选稿由 585 页降为 463 页。已经以实际首轮 PDF 重建并回填训练题和附录 E 的答案页码；最终回填版文本核验确认八章“第 X 章真题整理”、附录 F、150 个答案锚点都在，旧标题及两类页码占位符均为零。实际栅格抽查第四至第八章真题区和附录 F，公式、图形、页眉页脚与页码正常；第五章仅一题，因下一章必须另起而保留必要的章节末尾自然余白。另修正双线性变换公式被误标为“系统线性条件”的规则，现明确说明其用于建立模拟和数字频率间的非线性对应。全书、页码回填和 MathJax 回归共 `27 passed`。最新文字规则已写入 HTML；最终一次回填导出受当前 Codex 使用额度限制拦截，恢复后必须用新首轮 PDF 重新回填并对第六章公式总表作最终视觉核验，不能把此前 PDF 误作此文字版本的终稿。

> 2026-08-29 第八章第 1032 页二倍内插示例补全：该源页用 8 kHz 至 16 kHz 的二倍内插展示零插入产生镜像、抗镜像滤波器的通带增益和插值后谱。正文已具备可编辑的上采样—插值器结构，现补入该具体数值示例，完整以标准 LaTeX 给出 (L=2)、两种采样率、(h_i(n)=2\sin(0.5\pi n)/(\pi n)) 及抗镜像滤波器 `cases` 频响，并说明其用于消除 ​(pm\pi/2)​ 附近镜像。定向、第五至八章装配及全书正文回归 `10 passed`；静态 A4 导出 8 页，复核第 4--5 页：分段大括号自动伸缩、二倍内插公式与级联结构均完整，无黑块、裁切、源码或异常空白。至此本轮全书复核定位的第三、七、八章三页均已处理；下一步重新装配全书候选稿，再进行分区页眉／页脚和真题位置复核。

> 2026-08-29 第七章第 886 页线性相位推导补全：该页承载的是将关于 ​((N-1)/2)​ 成对的抽头写成系统函数、再在单位圆上分成偶对称余弦和与奇对称正弦和的完整过程。原正文虽有 (H_0(\omega)) 结论，但未保留该成对配对式与明确的两类分段频响；现补为完整 LaTeX 公式和 `cases` 环境，并在公式前说明它用于判断余弦／正弦项所对应的冲激响应对称性。回归先红后绿；第七章、第五至八章装配、全书正文装配及静态 MathJax 共 `16 passed`。实际 A4 栅格复核配对式、两行分段大括号、零极点图和相邻例题，均无源码、裁切、黑块、重叠或额外零轴。下一步核对第八章第 1032 页。

> 2026-08-29 第三章第 527 页五类傅里叶描述关系图补回：确认该源页不是动画重复，而是连续信号／序列／有限记录／周期序列／有限主值序列与 FT、DTFT、DFS、DFT 的五行对应关系。未截取含课件页眉、校标或水印的整页，而是用真实信号曲线、离散 stem、坐标轴和标准箭头重绘为可编辑 SVG；图内数学标签统一交由 MathJax 整体渲染。新增“可编辑五行对应图”回归先红后绿；第三章概览、第三至四章正文装配及全书正文装配共 `13 passed`。静态 MathJax／WeasyPrint A4 导出 3 页，实际核对第 1--2 页：图形完整、无黑块、水印、公式源码或裁切，正文自然续排。下一步继续核对尚未审清的第七章与第八章源页图形。

> 2026-08-29 第四章公式总表收录核对：按“每章末尾收录本章全部正文公式”的要求，发现 CZT 的无混叠长度 \(L\geq N+M-1\) 原先只作为行内说明出现，因而不会被公式总表提取。先为该遗漏加入总表回归并确认失败，再将长度条件提升为完整 LaTeX 展示公式，使它与 \(g(n)\)、\(h(n)\) 及两条分段补零式一同进入第四章公式总表，且由统一命名逻辑标明其计算用途。定向回归 `18 passed`；静态 MathJax／WeasyPrint A4 导出 12 页，目视复核第 11--12 页，全部公式、分段大括号和流程图完整；末页余白仅处于当前第四章正文组件的真正结尾，最终装配仍会继续衔接公式总表与训练。下一步继续第四章总装配／正文清理。

> 2026-08-29 第四章 CZT 卷积实现细节补全：继续核对课件第 649--652 页，发现原有正文虽已给出布鲁斯坦等式，却遗漏了将该等式真正落实为 FFT 的两条加权序列定义，以及消除循环混叠所需的长度 \(L\geq N+M-1\) 和两条补零／周期延拓序列。补入 \(g(n)\)、\(h(n)\)、\(g_L(n)\)、\(h_L(n)\) 的完整标准 LaTeX 定义，使用 `aligned` 与 `cases` 整体排版，并说明其分别用于卷积化和无混叠 FFT 计算。回归先红后绿为 `19 passed`；静态 MathJax／WeasyPrint A4 导出为 12 页，目视复核第 10--12 页：加权序列、分段函数大括号和流程图均完整，无源码、裁切、重叠或黑块；第 12 页为该组件末尾的自然留白，最终全章装配时仍会与公式总表和训练部分续排。下一步核对第四章总装配的公式总表收录，并继续正文优先的全章清理。

> 2026-08-29 第四章 CZT 图形补回与裁图复核：逐页核对课件第 648、652 页，识别出正文缺少两张承载独有算法信息的图：z 平面螺线取样路径，以及将卷积化计算落到加权、FFT、逐点相乘、IFFT 与最终加权的流程图。新增可复现裁图工具，裁取范围仅保留技术图形，排除课件标题条、校标、页码和底部解释横幅；正文分别紧邻 CZT 取样定义与布鲁斯坦卷积化说明，图注采用讲义自身编号。裁图边界测试先红后绿，定向回归 `19 passed`；静态 MathJax／WeasyPrint A4 导出为 11 页，目视复核第 9--11 页：两图完整、无讲义外模板与黑块，公式均整体渲染、无源码、裁切或重叠。下一步继续逐页核对第四章末段是否仍有未转入正文的唯一内容。

> 2026-08-29 第四章实序列 FFT 补全与蝶形图缩放修复：逐页核对课件第 639--643 页，补入将实序列奇偶样本组成半长复序列的完整定义、由一次 \(N/2\) 点 FFT 分离 \(X_1(k)\)、\(X_2(k)\) 的公式，以及实序列谱的共轭对称关系；每组公式均标明其使用目的。发现静态 A4 导出中 DIF 蝶形图按原始像素宽度溢出并裁去右侧输出，先加入装配级回归并确认失败，再为所有章节四级源图加入统一等比最大宽度约束。定向回归 `17 passed`；静态 MathJax／WeasyPrint A4 导出 10 页，目视复核第 7--9 页：DIF 蝶形图完整显示输入、全部级与输出，新增多行公式标准对齐、无源码、裁切、重叠或黑块。下一步继续对照第四章的源页核对其它算法图与文字是否有遗漏。

> 2026-08-29 第三章第 3.5 节频谱分析正文补全：逐页核对课件第 521--560 页，保留既有采样参数、混叠、泄漏、栅栏效应和 1024 点参数选取例题，补入矩形窗频谱公式及其主瓣首零间隔 \(4\pi/N\)、窗口长度对相对旁瓣起伏和过渡带的影响；补入 \(R_{32}(n)\)／\(R_{64}(n)\) 单一余弦比较，以及 \(R_{40}(n)\)／\(R_{320}(n)\) 下强弱相邻分量的分辨示例；补入零填充分段定义和“加密观察频点、但不改变真实频率分辨率”的界限。公式前均给出中文名称及用途，不纳入课件中 MATLAB 代码或实验内容。定向回归先红后绿为 `9 passed`；静态 MathJax／WeasyPrint A4 导出 34 页，目视复核第 20--23 页：竖式分数、分段函数大括号、中文说明与例题均完整，无公式源码、裁切、重叠或黑块。第 23 页为第三章正文结束页的可保留自然留白；导出仅报告既知的第四章两张 FFT 流程图资源缺失，未影响本次第三章验收。下一步核对第三章结尾公式总表的收录范围，再转入第四章正文源页核对。

> 2026-08-28 第三章第 3.4 节频域采样定义与正交关系补全：逐页核对源课件第 515--520 页，补入频域样值作为 (z) 变换在单位圆根上的取值 —— \\(\widetilde{X}(k)=\left.X(z)\right|_{z=W_N^{-k}}\\ —— 及其与 DTFT 坐标的等价式；同时补入根单位正交关系的标准 `cases` 表达，明确它用于化简 IDFT 双重求和并判定时域周期折叠位置。先以回归测试锁定公式和双反斜杠分行，再发现静态页面将分段条件压成一行；修正为完整 LaTeX `cases` 后定向回归 `8 passed`。MathJax 静态／WeasyPrint A4 导出 33 页，目视复核第 17 页：分段大括号覆盖两行、公式与中文用途说明完整，无源码、裁切、重叠或异常留白。导出仍仅报告既知第四章两张 FFT 流程图资源缺失，未影响本轮第三章频域采样验收。下一步继续核对第三章第 521 页及之后的唯一内容。

> 2026-08-28 第三章第 3.3 节分块卷积例题补全：逐页核对源课件第 495--514 页，确认既有正文已包含重叠相加／重叠保留的定义、长度条件和计算步骤；补入原例 \(x(n)=(n+1)R_8(n)\)、\(h(n)=R_3(n)\)，完整保留 \(M=4\)、\(N_2=3\)、\(L_0=6\) 的参数选择、重叠相加两段部分卷积与最终 \(\{1,3,6,9,12,15,18,21,15,8\}\)，以及重叠保留三块输入、每块前两点舍弃与有效输出拼接。新增公式均紧邻说明其计算用途。首次静态导出发现 \(\boldsymbol{}\) 会触发本地 MathJax 的按需加载错误，随即以“先红后绿”的回归把静态渲染器切换为本地完整 TeX 包，验证标准 AMS 粗体、矩阵和分段环境均可整体渲染。定向回归 `10 passed`；MathJax 静态／WeasyPrint A4 导出为 33 页，目视复核第 14--16 页，无公式源码、裁切、重叠或异常留白。导出仍只报告既知第四章两张 FFT 流程图资源缺失，未影响本轮第三章正文验收。下一步继续核对第三章第 515 页以后频域采样部分的唯一内容。

> 2026-08-28 第三章第 3.3 节圆周卷积长度与 DFT 自卷积例题补全：逐页核对源课件第 481--494 页，确认已有正文已含补零条件、循环卷积与线性卷积关系、重叠相加／保留法及 50/20 点无混叠区间例；补入同一对序列 \(x_1(n)=R_5(n)\)、\(x_2(n)=n+1\) 的线性卷积与 4、5、6、7 点圆周卷积完整对照，保留 \(\{7,8,9,6\}\)、\(\{6,6,6,6,6\}\)、\(\{4,3,6,6,6,5\}\)、\(\{1,3,6,6,6,5,3\}\) 及无混叠阈值；并补入 \(x(n)=\delta(n)+2\delta(n-2)+\delta(n-3)\) 的 4 点 DFT、自卷积结果与频域平方路线。新增公式均紧邻说明其用途。定向回归 `3 passed`；MathJax 静态／WeasyPrint A4 导出为 32 页，目视复核第 12--15 页，无公式源码、裁切、重叠或异常留白。导出仍只报告既知第四章两张 FFT 流程图资源缺失，未影响本轮第三章正文验收。下一步继续逐页核对第三章第 495 页及之后的唯一内容。

> 2026-08-28 第三章 DFT 圆周卷积例题补全：复核源课件第 461--480 页，确认现有正文已覆盖 DFT 的周期延拓、循环共轭对称与循环卷积定义；补入原例“\(x_1(n)=R_5(n)\)、\(x_2(n)=n+1\) 的 6 点圆周卷积”，完整保留题干、两列补零到主值区间、按 \(\left((n-m)\right)_6\) 取下标的卷积式和结果 \(\{4,3,6,6,6,5\}\)。全程沿用 \(((n))_N\) 周期下标记法，不出现 `mod N`。定向回归 `4 passed`；静态 MathJax／WeasyPrint A4 导出 31 页，目视复核第 9--11 页，例题、公式和相邻中文说明完整，无源码、裁切、重叠或异常留白。导出仍仅报告两张已知第四章 FFT 流程图资源缺失，未影响本次第三章验收。下一步继续逐页核对第三章第 481 页及之后的唯一内容。

> 2026-08-28 第三章傅里叶级数到 DFS 物理频率语义补全：复核源课件第 401--440 页及现有 DFS／DFT 组件。原有正文已覆盖 DFS 变换对、主值区间、周期延拓、DFT 定义、循环移位和例题；现补入“同一序号的傅里叶级数系数数值可相同，但实际物理频率仍由 \(k\Omega_0\) 与记录周期决定”的明确说明，避免只比较系数数值而忽略频率坐标。先新增定向断言并确认失败，再以最小正文补充转绿；第三章概览与正文装配回归 `4 passed`。静态 MathJax／WeasyPrint A4 导出为 31 页，目视复核第 1--3 页：中文说明、矩形脉冲列系数、余弦谐波展开、DFS 定义均完整，无公式源码、黑块、裁切或重叠。导出仍报告两张已知的第四章 FFT 流程图资源缺失，已单列后续图形修复队列，不与本次第三章正文验收混淆。下一步继续按源页核对第三章 DFT 定义与性质的余下唯一内容。

> 2026-08-28 第二章原课件第 303--319 页第 2.6 节一阶低通设计对账补全：保留一阶低通／高通、数字谐振器、DTMF、陷波、全通、最小相位及工程预处理的既有正文，补入实用一阶低通 \(H(z)=\frac{1-a}{2}\frac{z+1}{z-a}\) 的零极点配置、截止频率精确式 \(\omega_c=\arccos\!\left(\frac{2a}{1+a^2}\right)\)、原课件给出的典型带宽表和原例题的采样频率换算、\(a=0.9\) 系统函数及差分方程。新增零极点图、输入／输出 stem 图与离散频谱均由真实坐标和数据生成；原点仅在两轴交点右下方出现一次，未使用额外零轴。总装配层增加通用图像尺寸、题图题注和真实表格横竖线规则，避免组件样式被剥离后出现巨大空白或无框表格。定向回归先红后绿为 `4 passed`；静态 MathJax／WeasyPrint A4 正文为 28 页，目视复核第 21--28 页：表格线、公式、零极点图、stem 图、频谱图均完整，无黑块、源码、裁切、重叠或非章节末尾的异常大空白。下一步继续对照第二章第 320 页之后的唯一内容，并保持正文先行、真题待全书正文完成后再统一分章调用。

> 2026-08-27 第二章原课件第 282--302 页几何法对账补全：在已有通用零极点距离法之外，补回累加器 \(H(z)=z/(z-1)\) 的不稳定原因、把极点移至 \(0.9\) 后的稳定低通解释、在 \(z=-1\) 加零点形成高频衰减的具体关系；补回纯延时的恒定幅度／线性相位结论及四抽头平均 \(\frac12[x(n)+x(n-4)]\) 的四个单位圆零点和幅频响应。合并了重复标题／动画页，未带入课件模板、MATLAB 或程序代码。定向回归先红后绿为 `1 passed`；静态 A4 PDF 9 页复核第 6--9 页，分段函数大括号、竖式分数、零极点图、中文说明与版面均清晰，无黑块、源码、裁切或重叠。下一步转入第 2.6 节，并在最终总装配中继续自然续排。

> 2026-08-27 第二章原课件第 240--281 页系统函数与频率响应对账补全：补入一阶 LSI 系统正弦稳态响应例题的完整数学关系及数值结论、归一化角频率 \(\omega=\Omega T=2\pi/N_0\) 的换算、三点均值系统的冲激响应、频率响应、幅相性质和高频衰减解释；未纳入 MATLAB、音频读写或程序代码。新增公式均由本地 MathJax 整体排版。该组件静态 A4 PDF 共 8 页逐页复核：公式、单位圆图与中文说明完整，无公式源码、黑块、裁切或重叠；第 8 页是组件的暂时结束页，最终全章装配时须与后续正文自然续排，不能作为独立成品页保留大面积空白。定向回归 `1 passed`，差异检查通过。下一步继续对照第二章第 282 页及之后内容，并在总装配层完成自然分页。

> 2026-08-26 用户再次确认真题分区为全局硬规则：每章正文和公式总表之后紧接“第 X 章真题整理”，题目不得汇总到全书末尾；全部完整答案只置于书末“附录 F：真题整理详解”。章节题面区和书末详解区必须使用各自正确的大标题、页眉与连续页码页脚，不得出现“真题整理讲解序”或总真题区。本项已同步到永久上下文和需求变更记录；下一次全书重排须逐页复核。

> 2026-08-26 第二章系统函数与频率响应组件静态导出修复：定位到当前 MathJax→WeasyPrint 静态链只要页面内含 SVG（包括 data URI）即会错误放大同页公式的渲染器兼容问题；页面内不含 SVG 的最小对照组正常。现将该 z 平面图替换为 Matplotlib 根据真实坐标生成的 240 dpi PNG，再嵌入 A4 页面，保留单位圆、轴箭头、零点、极点、频率点及距离关系。定向回归通过；实际静态 A4 PDF 七页复核，公式无黑块、裁切、源码或异常放大，图中标签与图元清晰可读。下一步继续对照第二章第 240 页及后续原课件内容。

> 2026-08-26 第二章原课件第 223--239 页 DTFT 与共轭对称性对账补全：逐页核对 DTFT 正反变换、绝对可和存在条件、频域周期性、共轭对称／反对称定义、实虚部奇偶结构、时频对应关系、实序列频谱性质及原有例题。旧正文已有结论和例题，现补入复序列实虚部分解、实部序列 DTFT 的共轭对称推导，以及共轭分量对应频谱实部／虚部的完整多行推导。所有新增数学仍以本地 MathJax 的完整 LaTeX 整体渲染；第二章正文静态 MathJax／WeasyPrint A4 复核第 8--10 页，未见公式源码、裁切、重叠或不自然空白。相关组件、第二章正文、全书装配和全书导出回归为 `20 passed`。下一步继续逐页核对第二章第 240 页及之后内容。

> 2026-08-26 第二章原课件第 202--222 页反变换与性质对账补全：逐页核对围线积分、部分分式、幂级数、线性、移位、卷积及性质总表后，保留原有例题并补入重极点通式、四组标准反变换对、冲激移位变换与“性质—时域序列—z 域表达式—收敛域”完整对照表。所有读者可见数学均为完整 MathJax LaTex；表格在第二章总装配层重新施加横竖线及整行分页约束，避免局部样式在合书时丢失。定向回归与正文／全书装配回归为 `20 passed`。实际静态 MathJax／WeasyPrint A4 复核第 4--8 页：表格有完整线框，公式无源码、裁切或重叠，正文与 DTFT 模块自然续排。下一步继续逐页核对第二章第 223 页及之后内容。

> 2026-08-26 第二章原课件第 186--201 页开头对账补全：对照第二章导览、z 变换来源、s/z 平面映射、典型 ROC 与 ROC 可能性后，确认旧正文缺少由采样冲激列拉普拉斯变换导出 z 变换的完整关系，且四类 ROC 仅有文字。现补为标准 MathJax 多行推导及真实等比例复平面 ROC 图；右边、左边、双边 ROC 的阴影和极点圆均与定义一致，不采用课件标题条、校标或动画圈画。独立静态 MathJax／WeasyPrint PDF 实查三页：单位圆、ROC 图、公式和中文标签无黑块、裁切、重叠或游离注释。组件回归先失败后转绿；下一步继续核对原课件第 202--222 页的反变换和性质。

> 2026-08-26 第一章源课件第 101--185 页对账收束：已完成第一章余下源页至第二章开始页的逐页核对。补入标准“延时叠加系统”两支路求和框图、理想时域采样曲线与 stem 图，并补齐冲激列频域展开、连续/离散频率换算等唯一关系；所有新增公式均由本地 MathJax 整体渲染并带用途说明。定向回归（卷积性质、采样定理、第一章正文、全书装配）为 `19 passed`。重导出 533 页整书候选并实际查看第 29--46 页：反馈结构图箭头闭合正确，频谱、重构、采样链、零阶保持、公式总表均未见黑块、裁切、重复原点或公式源码。下一步：提交本轮第一章修复；随后以同一规则逐章对账第二章正文。

> 2026-08-24 第一章原课件第 81 页时不变性对账补全：旧正文只以 \(x(n)=n\) 给出缩放系统 \(y(n)=x(2n)\) 的抽象反例，漏掉原课件的有限序列 \(\{0,1,2,3,4,5\}\) 和 \(k=1,n=3\) 时 \(x(4)=4\)、\(x(5)=5\) 的直接对照。现已按原数据和运算步骤补回，保留“\(y(n)=x(2n)\) 为时变系统”的原结论。同步把三个仍要求 CDN `mathjax@3` 的过时测试改为本地 MathJax 入口。时不变性、线性、卷积、第一章正文与全书装配回归 `21 passed`；下一步继续对照第 101 页及之后的源课件。

> 2026-08-24 第一章原课件第 61--63 页周期性对账补全：逐页对照确认旧正文虽然保留周期定义和一般判据，却漏掉了整数周期例 \(A\cos(0.01\pi n)\) 的 \(N=200\) 推导、既约分数例 \(A\cos(3\pi n/7)\) 的 \(N=14\) 推导，以及 \(n\sin(\omega n+\varphi)\)、\(a^n u(n)\)、带非零实指数包络的复指数和单边正弦等无周期形式。现已按原题例和原结论补入；源页中的 MATLAB 绘图任务仍按全局规则不进入成品。周期性组件、第一章正文与全书装配回归 `19 passed`。下一步继续对照第 69--100 页的系统性质与卷积页，逐项补齐唯一推导、例题和必要图。

> 2026-08-24 第一章原课件第 11 页数列转写纠错：逐页对照发现当前讲义把原课件 \(x_3(n)=\{0,0,1,2,3,4,5\}\) 错漏为 \(\{0,0,1,2,3\}\)，违反“字不能漏”。已先把回归断言改为原课件完整数列，确认组件失败；随后修正正文模型并同步更新已经过时的本地 MathJax 路径断言。表示组件、第一章正文和全书装配相关回归 `22 passed`。随后以两遍静态 MathJax／WeasyPrint 重导出整书候选，仍为 533 页、`P.____` 为 0；实际查看成品第 2 页，完整数列已显示。第 9 页为章节封面、第 10 页为已覆盖的四种表示方式提纲；第 12--14 页的标准 stem 图、单位抽样序列、移位加权和均已进入现有组件，仍要继续视觉复核其实际 A4 页面。

> 2026-08-24 第一章原课件第 7 页对账补全：原候选只保留“连续信号经等间隔取样得到序列”的文字和公式，缺少连续时间曲线、样值与序列索引之间的关键对应图。现以真实连续曲线、采样 `stem` 与从 \(x_a(t)\) 到 \(x_a(nT)\)、\(x(n)\) 的映射链重建该图，未搬运课件标题条、校标、批注图形或其他模板元素。先新增缺图回归并确认失败，再实现后转绿；第一章局部 PDF 实测为 45 页，新增图自然进入第 1 页，采样率对比自然续至第 2 页，无黑块、公式源码、裁切或不必要空白。整书按两次静态 MathJax／WeasyPrint 排版和页码回填重导出，候选仍为 533 页；黑块 0、`P.____` 0，相关正文／全书／训练／页码回填回归 `26 passed`。整书代表页实查：第 1、2 页的新增图，第 59 页“第一章真题整理”及其“详解见 P.400”，第 399 页“附录 F：真题整理详解”均正确。密度扫描命中第 165 页的一张第二章真题相频图：该图本身清晰且不被裁切，但题页仍留有大量空白；后续需要依“单题一页”规则确认其题干／图是否可同页排入，不能误称为正常正文页的空白已全部解决。

> 2026-08-24 第一章源图对账修复：直接对照原课件第 5 页后发现，“不同采样频率下钢琴乐曲的赏析”在旧候选中被错误缩减为文字与频率列表，漏掉了连续波形和三组离散样值图。现以真实函数驱动的连续曲线及 `stem` 图重建四行对比，不保留课件标题条、校标、扬声器图标或水印。首次静态 A4 导出暴露 WeasyPrint 忽略 SVG CSS `fill/stroke`、使图元默认变黑的根因；已将图元改为内联矢量属性并在 PDF 实测确认无黑块。随后压紧视区，避免图整体被推到下一页造成开章页大空白。单章视觉复核与整书再导出后，候选仍为 533 页；黑块扫描、稀疏空页扫描、`P.____` 未回填扫描均为 0。相关源码／整书装配／训练放置／页码回填回归为 `25 passed`。候选：`full/outputs/数字信号处理讲义_图源队列完成候选.pdf`；这只是第 1 章首批原课件页对账进展，整书仍须逐批对账，不能视为最终定稿。
...
- Committed this milestone as `a5afb5b` (`Complete chapter two resonator and notch content`). The immediate push attempt was blocked by Windows credential manager with `SEC_E_NO_CREDENTIALS`; no GitHub upload is claimed until a later authenticated push succeeds.

# 2026-08-28 Chapter-two inverse-system and minimum-phase completion

- Audited the remaining Chapter 2 source pages through page 391. The source then starts Chapter 3, so the Chapter 2 body boundary is confirmed. Its final engineering-filter material is represented as limit filtering, median filtering, sliding average, trimmed mean, limit average and weighted average, with no MATLAB or courseware-template content.
- Restored the missing inverse-system definition and the source minimum-phase factorization example. The body now explains the use of the inverse relation \(H_i(z)=1/H(z)\), the cascade identity \(H(z)H_i(z)=1\), and the complete decomposition of \(H(z)=(1-3z^{-1})/(1-\frac34z^{-1})\) into the mathematically normalized minimum-phase and all-pass factors.
...
# 2026-08-30 Formula-purpose fallback audit, batch 17
...
# 2026-08-30 Source coverage audit: modern-filter background and Chapter 5 diagrams

- Filled the source pp.747--748 Wiener/Kalman background: historical-observation Wiener estimation, prediction--correction Kalman recursion, controllability/observability and the practical online-estimation motivation. The page 865 course closing message is intentionally outside the technical body.
- The new content was test-first: the chapter-six regression first failed, then passed. Chapter 2/5/6 components plus full assembly, answer-reference backfill and MathJax regressions passed `60 passed`; rebuilt full HTML contains the new prose.
- Rendered and visually reviewed all nine Chapter 5 A4 structure pages. Direct-I/II, transposed, cascade, parallel-resonator, FIR-tap, frequency-sampling and fast-convolution diagrams have no black fill or clipping; the two parallel-resonator branches both terminate at the summer. This is component-level evidence only, so the candidate PDF is deliberately unchanged pending full-book audit/export.

# 2026-08-30 Source coverage audit: concrete Chapter 5 filter examples

- Source p.664's second-order recursive-filter example was not present as its own equation. Added the editable difference equation \(y(n)=a_1y(n-1)+a_2y(n-2)+b_0x(n)\), with an explanation of how it maps to a block/signal-flow realization. Existing direct-I/II vector diagrams remain the corresponding structural illustrations.
- Source p.700's two-branch resonator coefficients were likewise absent as a concrete worked example. Added the two displayed second-order terms with coefficients \(0.6203,1.5685,0.9998\) and \(0.8671,0.9963,0.9998\), together with their conjugate-pole and finite-word-length interpretation; the existing two-branch summer diagram supplies the exact topology.
- Test-first regression first failed then passed. A local MathJax/WeasyPrint A4 inspection confirmed both formulas are readable, complete and adjacent to their explanatory prose and diagrams. Full relevant suite: `58 passed`; rebuilt full-handout HTML confirms both examples are present. Candidate PDF remains unchanged until the full coverage audit finishes.

# 2026-08-31 Formula-name specificity and local MathJax regression

- Replaced the residual generic `滤波器设计关系` formula lead across the assembled book with formula-specific names and uses: LSI convolution/frequency multiplication, passband/stopband attenuation definitions, all-pass properties, IIR second-order realizations, Butterworth prototype/pole/order relations, impulse-invariant and bilinear mappings, window methods, equiripple criteria, and band-filter responses. Rebuilt full-handout HTML has `FILTER_DESIGN_GENERIC_LABELS=0`.
- Test-first checks now require the Chapter 5 and 6 summaries to name their actual leading formulas and prohibit the generic label in both main-body and complete-handout assembly. Static local-MathJax regression (`build_all_main_body`, `prerender_mathjax_svg`, `build_full_handout`) passed `55 passed`.
- The former browser-only checks cannot currently run: both Edge and installed Chromium terminate in headless mode with a GPU-process fatal error (`GPU process isn't usable`, exit `0x80000003`) even with fresh temporary profiles. This is an export-environment fault, not a formula rendering result; the validated PDF candidate is intentionally not replaced until a full visual export can be repeated.

# 2026-08-31 Full formula-purpose export and stable page-reference verification

- Completed the full two-pass static MathJax export after formula-name specificity changes. The first 466-page pass supplied real answer anchors; its page references changed the final pagination by one page, so a second answer-anchor backfill and export was required.
- The second pass is stable: the final candidate is A4, 465 pages, carries all 150 answer anchors, has zero changed answer-anchor pages relative to the immediate predecessor, and contains no `P.____` or `P.待回填` placeholders. The verified file replaced `full/outputs/数字信号处理讲义_正文图形修复候选.pdf` (matching SHA-256).
- Representative 144-dpi render checks covered Chapter 3 formulas, the Chapter 3 training section and page references, the Chapter 5 IIR direct-I structure, the Chapter 7 zero-pole/symmetry table, Appendix F and the final self-check page. No formula source text, black blocks, clipping, table overflow or page-number loss was found. Complete final static regression: `57 passed`.
- Desktop/mobile browser preview remains blocked solely by the known headless GPU-process failure. Remaining work is still the source-by-source coverage/diagram audit and final release packaging; this candidate is not the claimed project final.
- Task commit `919add4` contains only this verified candidate PDF and its two task records. The subsequent `git push origin main` was attempted and failed with `SEC_E_NO_CREDENTIALS`; it is therefore local only and must not be described as uploaded until credentials are restored.

# 2026-08-31 Chapter-boundary reflow and FIR formula-purpose correction

- Source p.875's two linear-phase FIR symmetry derivations were still labelled with the generic “离散时间求和关系”. They now explicitly name the first-type real/imaginary comparison, first-type center-symmetry criterion, and second-type center-antisymmetry criterion, each with its reader-facing use.
- The Chapter 6 final 2025 question exposed a large blank page tail because consecutive chapter sections were forcibly paginated. The stylesheet now lets a new chapter follow the preceding chapter's final training question naturally; this removes the forced blank while preserving all content and makes the preliminary static export 463 pages instead of 466.
- Test-first regression: 50 passing tests across the main-body and full-handout suites. A 144-dpi visual reflow check of pp.228--230 confirms the former blank page is replaced by Chapter 7 content and the new formula labels render without clipping.
- The two-pass answer-page-reference check is stable: the second 462-page output has all 150 anchors, zero target-page changes after the second backfill, and zero `P.____`/`P.待回填` placeholders. The candidate is not yet replaced because a further formula-label batch has changed the assembled HTML.
...
## 2026-08-31 Chapter 2 source-page audit: frequency-response basics

- Source pp.257--268 were rendered and compared. The convolution-to-frequency multiplication relation, the DTFT definition of \(H(e^{j\omega})\), magnitude/phase decomposition, dB gain/attenuation interpretation, ideal-delay phase and group-delay relation, and the fixed-frequency complex-exponential/real-sinusoid output rules are present as complete editable MathJax formulas with reader-facing names and uses.
- The source's repeated colored derivation slides are consolidated into the single complete convolution theorem and the frequency-response relationship. The output explicitly gives both \(y(n)=H(e^{j\omega_0})e^{j\omega_0n}\) and the real-sinusoid amplitude/phase rule; the ideal-delay formula includes the group-delay result.
- The source frequency-domain block-flow drawings add no independent topology beyond the already covered LSI convolution/frequency multiplication relation. They are intentionally not copied as decorated screenshots. Source pp.267--268 are recap/outline pages; no new technical content is missing.

# 2026-08-31 Chapter 2 source-page audit: calculated frequency-response figures

- Source pp.269--281 contain the first-order low-pass example, the three-point averaging response, and the three-tone frequency-selection demonstration. Their equations and conclusions were already editable, but their explanatory response graphs had not been retained as clean calculated figures.
- Added three native Matplotlib figures driven by the actual system functions: a first-order magnitude/phase/group-delay plot, a three-point-average magnitude/principal-phase plot with a true phase discontinuity at the zero, and a three-frequency stem plot showing the computed filtered amplitudes. The source slides' MATLAB listings, watermark, and redundant cover frames are not imported.
- The first render exposed missing Chinese glyph warnings because Matplotlib fell back to DejaVu Sans. A test first reproduced the warning; the component now loads the verified local Microsoft YaHei font explicitly and avoids unsupported Unicode subscript glyphs. Image-level review confirms Chinese labels, arrow directions, curve continuity/discontinuity, stem formatting, and margins. Focused plus full relevant regression passes `78 passed`.
- This work changes the full HTML body but not the verified 462-page candidate yet. It awaits the later complete static-MathJax export, answer-reference backfill, and final raster audit.

# 2026-08-31 Chapter 2 source-page audit: frequency geometry and resonators

- Source pp.282--316 were rendered and compared. The accumulator/stable one-pole/high-frequency zero geometry, pure delay, comb filter, two-tap low/high pass, first-order cutoff relation, bandwidth table, and the 10 Hz/250 Hz low-pass example are already present in editable prose, MathJax, tables, native pole-zero diagrams, calculated response plots, and stem/spectrum figures.
- Source pp.317--328 exposed a remaining visual gap in the resonator section: the equations gave the pole-radius rule and a numerical band-pass design, but did not preserve the diagrams that make their frequency-selective action visible. Added two native Matplotlib textbook figures driven directly by the stated poles and transfer functions: a unit-circle pole pair with calculated radius comparison, and the exact band-pass magnitude response for the worked design. Both axes include arrows and data margins; the unit circle is equal aspect; Chinese labels use explicitly loaded Microsoft YaHei; no source screenshot, watermark, or slide decoration is imported.
- Test-first coverage failed before the images existed, then passed. Image-level review confirms pole positions, equal unit-circle geometry, arrows, zero-frequency endpoints, centre-frequency peak, Chinese labels, and no clipping or black region. The Chapter 2 component/body/full-handout/formula/static-MathJax suites pass `83 passed`. The candidate PDF still intentionally predates this batch.

# 2026-09-01 Chapter 2 source-page audit: DTMF and notch filters

- Source pp.329--352 were rendered and checked. The DTMF frequency-pair principle and the 50 Hz notch design were present mathematically, but the full keypad mapping and the source's zero-pole/frequency-response explanation were not present as clean reader-facing graphics in Chapter 2.
- Restored the complete 4-by-4 DTMF keypad mapping as a real bordered HTML table, including the low/high group frequencies and A--D extension keys. Added a real calculated 50 Hz notch figure from \(\omega_0=0.1\pi\) and \(r=0.95\): unit-circle zero/pole positions and its actual magnitude response. It uses equal z-plane axes, arrowed plot axes, margins outside data, Chinese labels, and no screenshot/watermark.
- The added test first failed because both artifacts were absent, then passed. Rendered PNG review confirms the unit circle remains circular, zero/pole symbols are distinct, the 50 Hz null is at the correct frequency, and no black region or clipping appears. Relevant component/body/full-handout/formula/static-MathJax suites pass `84 passed`. Candidate PDF remains intentionally unchanged pending the rest of the Chapter 2 audit.

# 2026-09-01 Chapter 2 source-page audit: all-pass and minimum-phase compensation

- Source pp.353--372 were rendered and compared. Restored the first-order all-pass section transfer function, its unit-circle response, exact phase relation, and positive group-delay relation. A calculated three-panel magnitude/phase/group-delay figure now makes the phase-correction role visible without importing source screenshots or slide watermarks.
- Source pp.365--371 additionally contain the loss-system compensation topology. Restored the minimum-phase factorization of a general stable distortion system, its stable compensation inverse, the remaining all-pass transfer function, and an editable left-to-right SVG block diagram. Every mathematical label within that diagram is rendered as one MathJax expression; its arrows terminate at the intended block inputs and outputs.
- A pre-existing formula-summary test exposed generic duplicate labels for the newly added all-pass equations. The root cause was missing semantic-name matches in the centralized formula-label resolver, not formula duplication. Added exact semantic mappings for the four all-pass formulas and the two compensation relationships. The source-first tests failed before each restoration and now pass. Static MathJax plus temporary A4 raster review confirmed complete braces, formulas, plots, Chinese labels, signal-flow arrows, no black region, and no clipping. Candidate PDF remains intentionally unchanged until the Chapter 2 audit is complete.

# 2026-09-01 Chapter 2 source-page audit: engineering-filter comparison

- Source pp.377--388 were rendered and compared. The existing prose already retained the limiting, median, sliding-average, trimmed-mean, and weighted-average principles, but it lacked the source's practical before/after comparisons that make their different impulse-noise behavior visible.
- Added one data-driven four-panel stem figure: a shared sampled signal with isolated impulses and the outputs of threshold limiting, three-point median filtering, and four-point causal sliding averaging. The first stem is deliberately separated from the y-axis; axes have arrows and data margins; labels use verified Chinese fonts. The explanatory copy states the distinct behavior of all three methods without importing source watermarks or nontechnical slide decoration.
- A source-first regression failed before the calculated comparison existed and now passes. Image-level review confirms all panels use discrete stems rather than continuous lines, the origin is not duplicated, labels are clear, and no clipping or black region occurs. Source page 389 begins the Chapter 3 Fourier-series material; this completes the Chapter 2 source-page audit interval through p.388.

# 2026-09-01 Chapter 3 source-page audit: Fourier-series foundations

- Source pp.389--400 were rendered and compared. The reader-facing overview already had the four-domain map and rectangular-pulse coefficient, but lacked the general continuous-time Fourier-series transform pair and an explicit data-driven comparison of finite harmonic reconstruction.
- Added the exact synthesis/analysis pair and explains that it is used to calculate harmonic coefficients from one period and reconstruct the periodic waveform. Added a native SVG plot generated from the rectangular-pulse Fourier coefficients, comparing the calculated 11- and 21-harmonic partial sums against the target pulse train. It preserves source concepts without importing slide chrome, logos, or watermarks.
- The new regression first failed, then passed. Static MathJax plus A4 raster inspection of all three pages confirms the formula uses a single MathJax display, axes have arrows, plots are calculated rather than screenshots, Chinese labels are readable, and there are no black regions, clipped boundaries, duplicate origin labels, or large artificial blank areas. Targeted Chapter 3/body/formula/static-MathJax regression passes `69 passed`.

# 2026-09-01 Chapter 3 source-page audit: FS-to-FT limit

- Source pp.401--412 then derives the continuous Fourier transform from the Fourier series as the period tends to infinity. The general FS pair and pulse example were already present after the preceding correction, but the scaling identity and Riemann-sum limit were absent.
- Added the frequency-grid relation, coefficient scaling, the exact sum-to-integral transition, and the continuous-time Fourier-transform analysis equation. The prose explains their role: translating discrete harmonic spacing and coefficients into a continuous spectrum, rather than presenting formulas without purpose.
- A source-first test failed before the bridge existed and now passes. Targeted overview/body/formula/static-MathJax regression passes `68 passed`; a fresh static MathJax A4 render confirms the two displays and FT analysis formula are intact, with no source text, black regions, formula clipping, or overlap. The component-only final page has only natural end-of-component whitespace; the final full-body assembly continues into subsequent components and must be checked after a larger export batch.

# 2026-09-01 Chapter 3 source-page audit: DFS and DFT circular operations

- Source pp.413--460 were compared. The DFS derivation, transform pair, periodicity, linearity, modulation, circular convolution, DFT definition, unit-circle sampling, and the 8/16-point DFT example already matched the reader-facing components, so they were not duplicated.
- Two genuine omissions were restored: an editable three-row stem diagram of periodic extension, two-sample circular shift, and principal-interval extraction; and the basic DFT pairs for impulse, shifted impulse, the N-point rectangular sequence, and the DFT-bin complex exponential. The diagram is generated from its eight sample values, labels its actual cyclic operation, uses arrowed axes and one origin label per row, and has no source screenshot or slide decoration.
- Test-first coverage failed before both artifacts existed and then passed. Static MathJax A4 raster review caught a label/data collision in the first SVG draft; labels were moved into the inter-row spaces and the final render has no overlap, clipping, black region, or duplicate zero axis. Targeted Chapter 3/body/formula/static-MathJax regression passes `66 passed`.

# 2026-09-01 Chapter 3 source-page audit: DFT symmetry and spectral-line example
...
# 2026-09-01 Chapter 3 source-page audit: sampling aliasing, windows, and zero padding
...
# 2026-09-01 Chapter 5 source-page audit: linear-phase folded realization

- Audited source pp.656--736: basic filter representations; IIR direct I/II, transpose, cascade and parallel forms; FIR tapped-delay, cascade, frequency-sampling, fast-convolution and linear-phase forms. Chapter 6 begins at source p.742; intervening pages are the nontechnical noise example and are not reader-facing course content.
...
- Published candidate: `full/outputs/数字信号处理讲义_2026-09-02_全书重排验收候选.pdf`; SHA-256 `3AAD59A7A57E3F6E23C8F1C37E39E13A5BACF6D2FF273CAC8F3F6C7BA559549B`. Commit `090bc6f` was pushed successfully to `origin/main`.
