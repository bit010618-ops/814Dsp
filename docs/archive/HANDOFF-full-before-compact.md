> 归档说明：这是低 Token 上下文重构前的完整交接记录；正文保留当时工作区版本的原始顺序与内容。
...
## Latest imported-full-generator baseline milestone — 2026-07-22

- 已找到并隔离导入上一工程 `4231653` 至 `legacy_full/`：其中含 13 个可修改的 ReportLab 批次生成器、数学与图形重绘辅助代码、稳定 119 页 PDF 和完整历史测试。当前仓库不再只有 5 页样稿和元数据清单，也保有可复现的全文成书代码基线。
...
## Latest appendix architecture decision — 2026-07-22

- 最终附录保留 A、B、C、D、E、F、I：A 为完整公式与变换对速查；B 为考研标准答题模板；C 为与正文例题和真题去重后的分章综合训练；D 为考前高频公式和计算题检查表；E 为按八章检索的华理 814 真题考点导航；F 为全部纳入真题单元的整理详解；I 为八章自测和最后两分钟检查。
- 不设置独立 G／H。有限字长、结构实现、快速卷积、音频和多采样率等只要属于源课件，即在相应正文中完整保留；超出课件的工程实验不独立成附录，避免偏离华理 814 考研主线并重复增加页数。

## Latest complete chapter-one content-model milestone — 2026-07-22

- `full/tools/build_chapter_01_content_model.py` 从固定的原章节边界生成 `full/source/chapter_01_content_model.json`。18 个单元按原课件顺序唯一覆盖源页 1–185：四个正式小节、全部 MATLAB 内容、采样恢复、模拟—数字处理链以及奈奎斯特／香农和视觉混叠课程案例均有明确落点，未将章节导览页或收束案例丢弃。
- 该模型把 20、35、129、130、171 绑定到 `chapter_01_direct_rewrites.json` 中已渲染核验的组件；源页 142–143、164–165 两组动画递进内容显式登记为分别保留。测试确认页号严格连续且无重复、直接重写与动画锚点一致、写盘输出与内存模型一致（`3 passed`）。
- 下一步不是宣布第一章完成：必须让实际 PDF 生成器消费这 18 个单元，逐项转写正文／公式／图形，并在章末追加已审计真题训练后再做视觉和覆盖验收。

## Latest global figure-clearance rule — 2026-07-22

- 图中任何文字、公式、坐标轴名称、刻度数字、图例与题注不得与坐标轴、stem 线、曲线、箭头、边框或其他文字重合。发现重合时，先给标签留净空再调整位置，且绝不改变样点、坐标、曲线、公式或知识含义。该规则适用于课件图、真题图、流程图、频谱图及所有后续可编辑图。
- 第一章采样组件已按该规则修正：横轴样点编号统一向右偏移 4 pt；四行波形比较的绘图区整体右移，并为宽公式行标预留 24 pt 净空。相应模型与测试已锁定该要求。

## Latest animation-state selection rule — 2026-07-22

- 动画递进页不能机械选最后一张。每组均选择技术信息完整、公式和图形本体最清晰的状态；最后一张仅在没有遮挡或降低可读性时才可采用。
- 如果后页的课堂方框、圆圈、箭头或批注遮挡／干扰公式，而前页已经含有完整知识内容，则采用前页的干净状态；后页唯一新增的知识必须以无圈画的讲义式文字、公式或图形补入。选择不得遗漏任何推导、条件、数值、单位、坐标、曲线或例题步骤，并须保留每组的选择依据。

## Latest chapter-one representation component milestone — 2026-07-22

- 源页 9–15 的“离散时间信号的表示方法”已转写为 `full/source/chapter_01_representation_component.json` 和 `full/tools/build_chapter_01_representation_component.py`，可重建为 3 页可编辑 PDF；源页 15 与源页 10 完全重复，按唯一安全合并记录保留其追溯关系。
- 300 dpi 逐页复核确认：数列中 `n=0` 的下划线位置、单位抽样序列的分段定义、任意序列的移位加权和与所有样点值完整。`delta`、不等号和图内公式标签全部采用正式数学渲染，且横轴名与最右刻度、刻度与 stem 线均留有净空。
- 组件只代表第一章自然续排的一个内容块；下一步必须与源页 1–185 的相邻正文合成，不能作为最终分页直接交付。
