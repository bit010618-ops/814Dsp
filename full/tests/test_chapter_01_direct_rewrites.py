...
def test_direct_rewrite_model_preserves_all_low_coverage_source_pages_and_key_content():
    ...
    assert [item["source_page"] for item in model["blocks"]] == [20, 35, 129, 130, 171]
    ...
    assert "function [x,n] = impseq(np,ns,nf)" in model["blocks"][1]["matlab_code"]
    assert r"h(n)=a^n u(n)" in model["blocks"][2]["result_formula"]
    assert r"h(n)=-a^n u(-n-1)" in model["blocks"][3]["result_formula"]
    assert model["blocks"][4]["signal_chain"]["blocks"] == [
