...
def test_prerender_replaces_a_complete_cases_formula_with_mathjax_svg(tmp_path: Path) -> None:
    ...
    assert 'mjx-container[display="true"] svg.mathjax-svg{max-width:100%}' in rendered
    assert 'mjx-container[display="true"] svg.mathjax-svg{max-width:100%;height:auto}' not in rendered
    assert 'width="' in rendered and 'pt"' in rendered
    assert 'height="' in rendered and 'pt"' in rendered
    assert 'width="5.07ex"' not in rendered
