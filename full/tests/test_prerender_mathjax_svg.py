...
def test_prerender_converts_static_math_inside_svg_foreign_object_to_an_embedded_mathjax_image(
    tmp_path: Path,
) -> None:
    source = tmp_path / "source.html"
    target = tmp_path / "target.html"
    ...
    assert '<image x="10" y="5" width="70" height="30"' in rendered
    assert 'href="data:image/svg+xml;base64,' in rendered
