...
def _formula_name(formula: str, heading: str) -> str:
    ...
    if "频分复用" in heading and "\\sum" in compact:
    ...
    semantic_topics = (
        (("给定极点时 ROC 的可能性",), "有理 z 函数的候选收敛域（用于由极点模值判断左边、双边或右边序列）"),
        (("窗口长度与泄漏",), "窗函数的频率响应（用于观察窗口长度对主瓣宽度和频谱泄漏的影响）"),
        (("简单一阶低通与高通",), "一阶低通滤波器的系统函数（用于通过极点位置控制低通平滑程度）"),
        (("重叠相加法验证",), "重叠相加法的输出合成式（用于把分块卷积的重叠样本相加为连续输出）"),
        (("对偶性",), "DFT 的对偶性质（用于交换时域序列和频域序列的角色）"),
        (("其他常用性质",), "z 变换的基本定义（用于把离散序列表示为 z 域函数）"),
        (("逆系统与最小相位条件",), "最小相位与全通因子的分解（用于把系统分离为可逆最小相位部分和全通部分）"),
        (("零填充的作用",), "零填充后的 DFT 取样关系（用于在不改变 DTFT 的前提下加密频率取样）"),
        (("高通、带通与带阻的理想单位抽样响应",), "理想高通 FIR 的单位抽样响应（用于由低通原型构造高通滤波器）"),
        (("规模为何会成为问题",), "直接 DFT 的计算量（用于说明长序列频谱计算的复杂度增长）"),
        (("频分复用",), "频分复用信号的叠加表达式（用于表示多个频带信号在同一通道中的合成）"),
        (("512 点 DFT 的计算时间",), "DFT 旋转因子定义（用于估算和组织 N 点频谱计算）"),
