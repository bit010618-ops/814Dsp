...
def page_two_preamble(page: canvas.Canvas, y: float, *, continuation: bool) -> float:
    y = section(page, "同一组样值未必对应唯一连续信号", y) if continuation else title(page, "同一组样值未必对应唯一连续信号", y)
    y = paragraph(page, "设离散序列 {{x(n)=sin(0.1\\pi n)}} 来自在 {{f_s=1000\\,Hz}} 下对某个连续正弦信号的采样。虽然样值的变化规律已经确定，但若不额外限制连续信号的频带，仅凭这些样值仍无法唯一判断原连续信号。", y)
    return formula_box(page, r"x(n)=\sin(0.1\pi n),\qquad T=\frac{1}{f_s}=1\ \mathrm{ms}", y, 56)


def chapter_close(page: canvas.Canvas, y: float) -> float:
    y = section(page, "透过现象看本质", y)
    ...
    return paragraph(page, "本章从离散时间信号的表示、运算与典型序列出发，讨论了系统性质、线性卷积、差分方程，以及采样、恢复和模拟—数字处理链。贯穿全章的判断原则是：先明确对象与条件，再使用相应的数学表示和系统关系。", y)
...
def _page_two_legacy(page: canvas.Canvas) -> None:
    start(page, 2)
...
def page_three_preamble(page: canvas.Canvas, y: float) -> float:
    y = section(page, "车轮现象：视觉中的混叠", y)
    y = paragraph(page, "摄像机以固定帧率记录转动的车轮，相邻帧之间只能看到有限次状态。当车轮在一帧间隔内转过接近一个辐条间距时，下一帧的图案会与上一帧十分相似，于是视觉上可能出现车轮静止或反向转动。", y)
    return section(page, "离散观察下的不同表象", y - 2)


def wheel_block(page: canvas.Canvas, y: float) -> float:
    """Draw the paired wagon-wheel figures as one indivisible reading block."""
    # Compact enough to keep the complete explanation on the same page, while
    # still printing at a readable 4.2 cm diameter.
    wheel(page, 180, y - 98, 60, 90)
    wheel(page, 420, y - 98, 60, 45)
    page.setFillColor(BLUE)
    page.setFont(style.FONT_SANS, 11)
    page.drawCentredString(180, y - 180, "帧间转过一个辐条间距：看起来近似静止")
    page.drawCentredString(420, y - 180, "帧间略少转一些：看起来反向转动")
    return y - 207
...
def page_two(page: canvas.Canvas) -> None:
    ...
    y = formula_box(page, r"\left.\sin(2100\pi t)\right|_{t=nT}=\sin(2.1\pi n)=\sin(0.1\pi n)", y, 54)
    y = paragraph(page, "前者的频率为 {{50\\,Hz}}，后者的频率为 {{1050\\,Hz}}；在该采样频率下，它们的样值完全相同。这正是频谱复制造成的歧义：连续信号必须先满足带限条件，才能从样值中唯一恢复。", y)
    y = page_three_preamble(page, y - 18)
    y = wheel_block(page, y)
    y = paragraph(page, "该现象与信号采样中的混叠本质相同：连续变化被离散时刻观察后，原本不同的变化过程可能映射为难以区分的离散序列。提高观察频率或限制原信号带宽，才能消除这种歧义。", y - 10)
    chapter_close(page, y - 12)
    page.showPage()


def _page_three_legacy(page: canvas.Canvas) -> None:
    start(page, 3)
