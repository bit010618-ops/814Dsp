...
def _page_echo(page: canvas.Canvas, model: dict) -> None:
    ...
    # The next topic begins here at a safe boundary, using the lower page
    # space instead of opening an almost empty standalone page.
    y = style.draw_title(page, "序列的反褶", y - 20)
    y = style.draw_rich_paragraph(page, "序列 {{x(n)}} 的反褶序列定义为 {{y(n)=x(-n)}}。它相当于把横轴索引的正负号互换，因此以 {{n=0}} 为对称轴；{{x(0)}} 在反褶后保持不变。", 62, y, A4[0] - 124)
    _formula_box(page, r"y(n)=x(-n)", y - 3)
    page.showPage()


def _page_reversal(page: canvas.Canvas) -> None:
    _start(page, 3)
    _draw_stems(page, 72, 520, 200, 130, [[0, 1], [1, 2], [2, 3]], n_min=-3, n_max=3, label="x(n)")
    _draw_stems(page, 325, 520, 200, 130, [[-2, 3], [-1, 2], [0, 1]], n_min=-3, n_max=3, label="x(-n)")
    page.setFillColor(HexColor("#52616B"))
    page.setFont(style.FONT_SANS, 8.4)
    page.drawCentredString(172, 505, "原序列：样值索引为 0、1、2")
    page.drawCentredString(425, 505, "反褶后：索引变为 0、−1、−2")
    y = _operation_heading(page, "周期序列与反褶后的移位", 478)
    ...
    # Carry the next section's title and lead into the free lower space.
    y = style.draw_title(page, "累加和与差分", y - 20)
...
def _page_accumulation_difference(page: canvas.Canvas) -> None:
    _start(page, 4)
    y = _operation_heading(page, "例：矩形序列的后向差分", 774)
    ...
    y = style.draw_title(page, "时间尺度、能量与平均功率", y - 20)
    y = _operation_heading(page, "时间尺度（比例）变换", y)
    style.draw_rich_paragraph(page, r"当 {{m>1}} 为正整数时，{{x(mn)}} 为抽取（下采样）序列，只保留原序列每隔 {{m}} 个索引的样值；{{x(\frac{n}{m})}} 为插值（上采样）序列。", 62, y, A4[0] - 124)
    page.showPage()


def _page_scale_energy_power(page: canvas.Canvas) -> None:
    _start(page, 5)
    y = _formula_box(page, r"x(n)=\{0,1,2,3,4,5,6\}\ \Longrightarrow\ x(2n)=\{0,2,4,6\}", 774)
