...
def _page_representation(page: canvas.Canvas) -> None:
    _start(page, 2)
    # Keep the title fully below the shared header rule.  A 752 pt baseline
    # still intersects the rule after final continuous composition.
    y = style.draw_title(page, "离散时间信号的表达", 730)
    y = style.draw_rich_paragraph(
        page,
