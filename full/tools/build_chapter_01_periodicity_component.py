...
def page_two(page, m):
    ...
    y = para(page, "结论：{{12\\pi}} 为无理数，不存在整数 {{N}} 使 {{N\\omega=2k\\pi}}，所以该复指数序列不是周期序列。", y - 2)
    y = title(page, "周期求解方法与调幅序列", y - 12)
    ...
    para(page, "对 {{\\sin(\\omega_1n)\\sin(\\omega_2n)}}，先利用积化和差公式得到 {{\\omega_a=|\\omega_1+\\omega_2|}} 与 {{\\omega_b=|\\omega_1-\\omega_2|}}，再取各周期的最小公倍数。含 {{n}} 的系数或非零实指数包络会破坏周期性。", y)
    page.showPage()

def page_three(page, m):
    start(page, 3)
    y = section(page, "周期求解方法与调幅序列（续）", 746)
