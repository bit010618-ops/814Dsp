...
def _page_methods(page: canvas.Canvas, model: dict) -> None:
    _start(page, 1)
    # Reserve clear space below the common header rule for the first heading.
    y = style.draw_continuation_title(page, "用数列与函数表示（续）", 752)
    ...
    # Lift the complete stem-plot block into the available body area.  Its
    # height remains intact, so axes and lower labels are never cropped.
    _draw_stems(page, 74, 270, A4[0] - 148, 150, values, n_min=-1, n_max=11)
    page.showPage()
