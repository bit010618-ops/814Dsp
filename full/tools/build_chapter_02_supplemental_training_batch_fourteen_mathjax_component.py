...
def training_html() -> str:
    b = "\\"
    return f'''<section class="exam-page"><h1>第二章 补充真题（续）</h1><div class="exam-head"><span>2013、2015 年真题</span><span>详解见 P.____</span></div><p>七、离散因果 LTI 系统的系统函数 {b}(H(z){b}) 的零极点图如图所示，其中 {b}(h[0]=2{b})</p><figure>{pole_zero_svg()}<figcaption>系统函数的零极点分布。</figcaption></figure><p>（1）求系统函数 {b}(H(z){b}) 及收敛域；（2）判断是否稳定；（3）求单位脉冲响应 {b}(h(n){b})；（4）求出系统的差分方程。</p></section>'''
