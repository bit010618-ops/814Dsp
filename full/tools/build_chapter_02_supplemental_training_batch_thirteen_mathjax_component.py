...
def training_html() -> str:
    return r'''<section class="exam-page"><h1>第二章 补充真题（续）</h1><div class="exam-head"><span>2006 年真题</span><span>详解见 P.____</span></div><p>十、求如下图所示的 \(X(e^{j\omega})\) 所对应的序列 \(x(n)\)。图中频谱为偶函数，且一个主值区间内的幅值为：</p><div class="formula">\[
X(e^{j\omega})=
\begin{cases}
2, & \left|\omega\right|\le\frac{\pi}{4},\\
1, & \frac{\pi}{4}<\left|\omega\right|\le\frac{3\pi}{4},\\
0, & \frac{3\pi}{4}<\left|\omega\right|\le\pi.
\end{cases}
\]</div><p>并以 \(2\pi\) 为周期延拓。</p></section>'''
...
def answers_html() -> str:
    return r'''<section><h1>真题整理详解（续）</h1><h2>2006 年真题：分段幅度谱的 DTFT 反变换</h2><p>先把图形拆成两个以零频率为中心的矩形频带：宽度为 \(3\pi/2\) 的单位幅度矩形，再叠加宽度为 \(\pi/2\) 的单位幅度矩形。</p><p class="formula-name">离散时间傅里叶反变换积分（用于由给定频谱恢复离散序列）</p><div class="formula">\[
x[n]=\frac{1}{2\pi}\left(
\int_{-3\pi/4}^{3\pi/4}e^{j\omega n}\,\mathrm{d}\omega+
\int_{-\pi/4}^{\pi/4}e^{j\omega n}\,\mathrm{d}\omega
\right).
\]</div><p>当 \(n=0\) 时，两个矩形频带的面积相加，得到 \(x[0]=1\)。当 \(n\ne0\) 时，分别计算两个积分并相加：</p><p class="formula-name">分段频谱的反变换结果（用于给出完整离散序列）</p><div class="formula">\[
x[n]=\begin{cases}
1, & n=0,\\
\displaystyle\frac{\sin\left(\frac{3\pi n}{4}\right)+\sin\left(\frac{\pi n}{4}\right)}{\pi n}, & n\ne0.
\end{cases}
\]</div><p>该结果为实偶序列，这与题图给出的实偶幅度频谱一致。</p></section>'''
