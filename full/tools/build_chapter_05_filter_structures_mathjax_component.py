...
<p>二阶递归滤波器的差分方程（用于从输入、两个历史输出和反馈系数直接确定当前输出）为：</p>
<div class="formula">\[
y(n)=a_1y(n-1)+a_2y(n-2)+b_0x(n).
\]</div>
<p>该式可画成方框图，也可画成信号流图；两种表示都必须逐条保留相同的延时、增益和求和关系，而不能只凭外观替换信号方向。</p>
...
<p>双音并联谐振器的二阶节展开（用于把两组目标角频率的共轭极点分别配置到两条并联支路）可写为：</p>
<div class="formula">\[
\begin{aligned}
H(z)=H_1(z)+H_2(z)
&=\frac{0.6203z^{-1}}{1-1.5685z^{-1}+0.9998z^{-2}}\\
&\quad+\frac{0.8671z^{-1}}{1-0.9963z^{-1}+0.9998z^{-2}}.
\end{aligned}
\]</div>
<p>每个二阶节对应一对共轭极点；并联求和后可同时突出两组频率。实现时仍要检查有限字长使极点半径偏移后对稳定性和频率选择性的影响。</p>
<figure>
__DTMF_PARALLEL__
