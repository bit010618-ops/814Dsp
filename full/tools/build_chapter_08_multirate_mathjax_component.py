...
<p><strong>CD 音频兼容的采样率转换。</strong>CD 播放标准采用 44.1 kHz、16 bit；母带处理常在 48 kHz、96 kHz 或 192 kHz 以及 24 bit 等格式下进行。因此交付 CD 前，采样率转换器必须完成到 44.1 kHz 的非整数倍采样率转换；随后再将位深由 24 bit 降为 16 bit。前者处理时间采样密度，后者处理量化精度，二者必须分开完成。</p>
