...
def document(latex: str, *, title: str = "数学公式") -> str:
...
def write_html(latex: str, html_path: Path, *, title: str = "数学公式") -> Path:
    html_path.parent.mkdir(parents=True, exist_ok=True)
    html_path.write_text(document(latex, title=title), encoding="utf-8")
    return html_path


def render_pdf(latex: str, output_path: Path, *, title: str = "数学公式", wait_ms: int = 8000) -> Path:
    ...
    write_html(latex, html_path, title=title)
